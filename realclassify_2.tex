...
\begin{abstract}
In \citet{AVG1985} singularities is classified using normal forms. Using this classification equivalence classes of singularities are not always uniquely determined. We present the complete structure of the
equivalence classes of the unimodal real singularities of corank~$2$, i.e.\@ we
explicitly answer the question when the images of two parameters under two normal forms are equivalent and
...
Recall \citep[cf.][]{AVG1985} that a normal form is a smooth map of a finite dimensional linear space of parameters $\bf B$ into the space of polynomials such that, among other conditions, the inverse image in $\bf B$ of each orbit is finite. This is indeed the case for the normal forms in Table~\ref{tab:normal_forms}, as is clear from the results in this paper. The question is in which cases do the inverse image of an orbit have more than one element, i.e. when are two polynomials for two different values of the parameter in the image of a normal form equivalent. Similarly the question can be ask when the images of two parameters under two normal forms, respectively, are equivalent. Such equivalences occur only if the two normal forms are of the same main type. However, there are no equivalences between different main types. To give
an example, $x^4-4x^2y^2+y^4$, the normal form of $X_9^{++}$ putting $a = -4$, is
equivalent to $-x^4+10x^2y^2-y^4$, the normal form of $X_9^{--}$ putting $a = 10$,
...
interested in the set of all pairs of values $(u, v)$ such that $g_1(u)$ is equivalent to
