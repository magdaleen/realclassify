...
awarded by Gert-Martin Greuel and Wolfram Decker. We are thankful to all of them.}
...
\begin{abstract}
In \citet{AVG1985} singularities are classified using normal forms. Using this classification normal form polynomials\footnote{\ref{normalform}} (see Introduction) as representatives of equivalence classes are not always uniquely determined. We present the complete structure of the
equivalence classes of the unimodal real singularities of corank~$2$, i.e.\@ we
explicitly answer the question when two normal form polynomials of different type are equivalent and
how the value of the parameter in a normal form can be transformed within the same equivalence
...
Recall \citep[cf.][]{AVG1985} that a normal form, regarding the classification of singularities, is a smooth map of a finite dimensional linear space of parameters $\bf B$ into the space of polynomials such that the image of $\bf B$ intersects all the orbits of the $\mu$-constant stratum $\bf K$ under consideration; the inverse image in $\bf B$ of each orbit is finite; and the inverse image of the whole complement to $\bf K$ is contained in some proper hypersurface in $\bf B$. It hence follows that for each equivalence class covering $\bf K$ there is at least one and are at most finitely many representatives in the image of $\bf B$, as is confirmed for the normal forms in Table~\ref{tab:normal_forms} by the results in this paper. Throughout this paper, for the sake of simplicity, we call the polynomials in the image of $\bf B$ under the normal form of type $T$ the normal form polynomials\footnote{In some literature normal form polynomials are also refered to as normal forms.}\label{normalform} of type $T$. The question is in which cases do the inverse image of an orbit have more than one element, i.e.~when are two normal form polynomials of the same type equivalent. Similarly the question can be ask when two polynomial normal forms of different types are equivalent. Such equivalences occur only if the two normal form polynomials are of the same main type. However, there are no equivalences between different main types. To give
an example, the normal form polynomial $x^4-4x^2y^2+y^4$ of type $X_9^{++}$, i.e.~the image of $-4$ under the normal form of type $X_9^{++}$, is
equivalent to the normal form polynomial $-x^4+10x^2y^2-y^4$ of type $X_9^{--}$
...
$T_2$, respectively, where $a$ denotes the parameter, then we are
interested in the set of all pairs of values $(u, v)$ such that the normal form polynomials $g_1(u)$ and
$g_2(v)$ are equivalent. This question can be asked in three different ways: If we consider
...
For the unimodal singularities, we add the value that we assign to the parameter which occurs
in the normal form as given in Table~\ref{tab:normal_forms} in parentheses to
the name of the singularity (sub-)type if we want to refer to the specifically to the corresponding equivalence class. For instance, we denote by $E_{14}(3)$ the
...
$\NF(E_{14}(a))(b) = \NF(E_{14}(b))$, $b\in\K$.
\end{remark}
...
\begin{defn}
Let $M_x^i$ and $M_y^j$, $i,j=1,2$, be sets of monomials in $\C[[x,y]]$ and let $\C\! M_x^i$ and
$\C\! M_y^i$ be the $\C$-vector spaces spanned by these sets, i.e.\@
$\C\! M_x^i := \bigoplus_{m \in M_x^i} \C m$ and analogously for $\C\! M_y^j$. We
define the set of coordinate transformations spanned by $M_x=M_x^1\cupdot M_x^2$ and $M_y=M_y^1\cupdot M_y^2$ as
\[
S(M_x, M_y) := \{ \phi \in \Aut_{\C}(\C[[x,y]]) \mid
\phi(x) \in \C\! M^i_x,\; \phi(y) \in \C\! M^j_y, \; i,j\in\{1,2\}\} \,.
\]
If $M_x^2=\varnothing$ we only write $M_x=M_x^1$. Similarly we only write $M_y=M_y^1$ if $M_y^2=\varnothing$.
\end{defn}
...
is a sufficient set of coordinate transformations for
...
& $Y_{r,s}$, $r\neq s$  & $\{x\}$ & $\{ y\}$ \\  \cline{2-4}
& $Y_{r,s}$, $r= s$  & $\{x\}\cupdot\{y\}$ & $\{x\}\cupdot\{ y\}$ \\ \hline
...
Note that in the case for $T=Y_{r,s}$, $r=s$, the variables $x$ and $y$ can be interchanged without changing
the singularity type. This makes this case unique to the other hyperbolic cases, in the sense that this is the only case in which $M_x^2\neq\varnothing$ and $M_y^2\neq\varnothing$.
\end{remark}
...
\phi(f) &= \phi_0^{w_2}(f)
+\myunderbrace{(\phi-\phi_0^{w_2})(f)}{=}{: R_2 \in E_{>d}^{w_2}} \\[.5ex]
\phi(f) &= \phi_0^{w_0}(f)
...
$\phi(x) = t_1 \cdot x + (\text{other terms})$ (or of the form $\phi(x)=t_1 \cdot y+(\text{other terms})$ in case $T=Y_{r,s}$, $r=s$).
...
{
\tY_r^+\left(a\right)         &       & \tY_r^+\left(\pm\sqrt{\zeta}a\right) \\
                 & \text{\huge$\circlearrowleft$} &                          \\
Y_{r,r}^{++}\left(ca^2\right) &       & Y_{r,r}^{++}\left(\zeta ca^2\right)  \\
};
\draw[<->] (m-1-1) -- (m-1-3);
\draw[<->] (m-1-3) -- (m-3-3);
\draw[<->] (m-3-3) -- (m-3-1);
...
$X_9^{++}$ & $X_9^{++}$ &
\multirow{6}{*}{$\Gamma_{\C'}\bigl(f_1^{1,1}, \ldots, f_6^{1,1}\bigr)$} &
\multirow{6}{*}{$\Gamma_{\R'}\bigl(f_1^{1,1}, \ldots, f_6^{1,1}\bigr)$} &
\begin{tabular}[x]{@{}l@{}}
...
singularities. (To be precise, for $Y_{r,s}$, $r=s$, we also have to take into account
