...
\begin{algorithm}[h]
\caption{\label{E_6} Algorithm for the case $E_6$}
\begin{algorithmic}[1]

\REQUIRE{$g\in \m^3\subset\mathbb Q[x,y]$ of complex
singularity type $E[6]$.}

\ENSURE{ the real singularity type of $g$, i.e.~$E_6^-$
or $E_6^+$.}

\STATE $h:= \jt(g,3)$
\STATE $s:=$ coefficient of ${x^3}$
\IF{$s=0$}
\STATE swap the variables $x$ and $y$
\STATE $s:=$ coefficient of ${x^3}$
\ENDIF
\STATE Factorize $h$ in linear factors over $\mathbb Q[x,y]$, with a factor
$g_1=b_0x+b_1y$
\STATE Apply (to $g$) $x\mapsto \frac{x-b_1y}{b_0}$, $y\mapsto y$
\STATE Write $g$ as $g=cx^3+d y^4+$ terms of degree $4$
