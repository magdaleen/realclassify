...
and~\ref{subsec:determinacy}, and we also give algorithms how these invariants
...
of $\langle G \rangle = (\m^2 J)$ (cf.\@ \cite{GP2008},
Corollary~A.9.7) and can thus also be computed by combinatorial means with the
...
not decrease under complexification (cf. \cite{AVG1985}, pp.~273-274). So by
...
variables. If the inertia index is equal to the number of variables in the base
ring, $f$ is of type $A_1^-$. Lastly, if the inertia index is $0$, then $f$ is
of type $A_1^+$.
...
The two cases can be distinguished by dehomogenization; the details are carried
out in algorithm~\ref{alg:D_4}. Since the determinacy of $D_4$ is $3$, it
suffices to look at the $3$-jet. If we want to dehomogenize the 3-jet via
$x\mapsto x$, $y\mapsto 1$ without reducing its degree, we first have to make
sure that the coefficient of $x^3$ is non-zero. It is easy to check that this
is achieved by lines 2 to 13 of algorithm~\ref{alg:D_4}. Under this condition,
dehomogenization does not change the number of factors, so the number of real
roots after dehomogenizing the 3-jet is an invariant of the real
subtype which is 1 in the case $D_4^+$ and 3 for $D_4^-$.
...
Lemma~\ref{x^3}, we can transform $g$ into a polynomial of the form
